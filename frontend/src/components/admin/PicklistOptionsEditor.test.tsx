...
describe('PicklistOptionsEditor', () => {
  ...
  it('deletes an option', async () => {
    ...
    const deleteBtn = screen.getAllByRole('button').find(btn => btn.querySelector('svg[data-testid="DeleteIcon"]'));
    if (deleteBtn) fireEvent.click(deleteBtn);

    await waitFor(() => {
      expect(metaApi.deleteOption).toHaveBeenCalledWith(fieldId, 'opt1');
